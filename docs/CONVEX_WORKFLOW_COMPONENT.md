...
The workflow must be deterministic, so it should implement most of its logic by calling out to other Convex functions. We restrict access to some non-deterministic functions like fetch and crypto. Others we patch, such as console for logging, Math.random() (seeded PRNG) and Date for time.
