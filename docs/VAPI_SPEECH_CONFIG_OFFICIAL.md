# Vapi Speech Configuration - Official Documentation

**Source**: https://docs.vapi.ai/customization/speech-configuration  
**Saved**: January 1, 2026  
**Purpose**: Reference copy for Phase 7.1 implementation
...
> **Note**: Speech speed can be controlled, but only PlayHT currently supports this feature with the `speed` field. Other providers do not currently support speed.
...
> **Note**: Currently, these configurations can only be set via API.
...
You can set how long the assistant waits before speaking after the customer finishes. The default is 0.4 seconds, but you can increase it if the assistant is speaking too soon, or decrease it if there's too much delay.
...
This feature uses advanced processing to detect when the customer has truly finished speaking, especially if they pause mid-thought.
...
Customize how the assistant determines that the customer has stopped speaking based on what they're saying. This offers more control over the timing.

**Example:** When a customer says, "My account number is 123456789, I want to transfer $500."
...
### Words to Stop Speaking

Define how many words the customer needs to say before the assistant stops talking. If you want immediate reaction, set this to 0. Increase it to avoid interruptions by brief acknowledgments like "okay" or "right".

**Example:** While setting an appointment with a clinic, set `numWords` to 2-3 words to allow customers to finish brief clarifications without triggering interruptions.

### Voice Activity Detection

Adjust how long the customer needs to be speaking before the assistant stops. The default is 0.2 seconds, but you can tweak this to balance responsiveness and avoid false triggers.
...
Control how long the assistant waits before starting to talk again after being interrupted. The default is 1 second, but you can adjust it depending on how quickly the assistant should resume.
...
### Customer Style
Think about whether the customer pauses mid-thought or provides continuous speech. Adjust wait times and enable smart endpointing as needed.

### Background Noise
If there's a lot of background noise, you may need to tweak the settings to avoid false triggers. Default for phone calls is 'office' and default for web calls is 'off'.
...
### Conversation Flow
Aim for a balance where the assistant is responsive but not intrusive. Test different settings to find the best fit for your needs.
...
```json
{
  "stopSpeakingPlan": {
    ...
    "acknowledgementPhrases": [
      ...
      "understood", "yeah", "yes", "uh-huh", "mm-hmm", "gotcha", "mhmm",
...
|-------|---------|-------------|
| `waitSeconds` | 0.4 | Seconds to wait before responding |
| `numWords` | 0 | Words needed to interrupt (0 = immediate) |
| `voiceSeconds` | 0.2 | Seconds of speech to trigger interrupt |
| `backoffSeconds` | 1 | Seconds to wait before resuming after interrupt |
| `smartEndpointingEnabled` | false | Use smart endpointing |
| `backgroundSound` | "off" (web) / "office" (phone) | Background noise setting |
...
|----------|------|----------|-------------|
| **LiveKit** | Text-based | English conversations | Most sophisticated, `waitFunction` tuning |
| **Vapi** | Text-based | Non-English | Alternative to LiveKit |
| **Krisp** | Audio-based | All languages | Analyzes pitch, rhythm, intonation |
| **Deepgram Flux** | Audio-text | Low latency needs | Built-in turn detection |
| **Assembly** | Audio-text | Simple setup | Uses transcriber's EOT flag |
